...
def main() -> None:
    ...
    expertise_of = {
        r["researcher_id"]: f"{r.get('expertise', '')} {r.get('department', '')}"
    ...
    for _pid, w in enumerate(
        sorted(all_works, key=lambda x: x.get("cited_by_count", 0), reverse=True),
        start=1,
    ):
        work_topics = topics_for_work(
            w.get("title", ""), w.get("concepts", []), topic_ids
        )
        oa_topic_names = [
            t.get("display_name", "") for t in (w.get("topics") or [])[:3]
        ]
        ...
        for order, a in enumerate(authorships, start=1):
            ...
            link = (
                rid is not None
                and author_at_bahria(a)
                and expertise_field_guard(
                    oa_topic_names or [t["topic_name"] for t in work_topics], guard_text
                )
            )
            ...
            authors.append(
                {
                    "researcher_id": rid if link else None,
                    "full_name": disp,
                    "order": order,
                }
            )
    ...
    for r in thin:
        for rec in s2_works_for(r["full_name"]) + crossref_works_for(r["full_name"]):
    ...
    if submitted_path.exists():
        ...
        have_titles = {
            re.sub(r"[^a-z0-9]", "", p["title"].lower()) for p in publications
        }
        merged = 0
        for s in submitted:
    ...
    topics = [
        {
            ...
            "researcher_count": sum(
                1 for r in researchers if name in r["research_areas"]
            ),
            "publication_count": sum(
                1 for p in publications if name in (p.get("topic_names") or [])
            ),
        }
    ...
    for r in researchers:
        r["topics"] = [
            {"topic_id": name_to_id[n], "topic_name": n} for n in r["research_areas"]
        ]
    for p in publications:
        p["topics"] = [
            {"topic_id": name_to_id[n], "topic_name": n}
