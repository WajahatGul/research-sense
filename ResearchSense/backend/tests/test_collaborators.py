...
def test_rare_shared_area_scores_higher_than_common():
    ...
    freq = {1: 4, 5: 2}
    assert score_of(copub=0, shared_ids={5}, topic_freq=freq) > score_of(
        copub=0, shared_ids={1}, topic_freq=freq
    )
...
def test_international_flag_from_candidate_collabs(repo):
    echo = next(c for c in repo.collaborators(1) if c["researcher_id"] == 5)
