...
def _fetch_crossref(doi: str) -> dict | None:
    ...
    try:
        resp = httpx.get(
            f"{CROSSREF_API}/{doi}", headers=HEADERS, timeout=30, follow_redirects=True
        )
    ...
    pub_type = "conference" if msg.get("type") == "proceedings-article" else "journal"
...
def _fetch_datacite(doi: str) -> dict | None:
    ...
    try:
        resp = httpx.get(
            f"{DATACITE_API}/{doi}", headers=HEADERS, timeout=30, follow_redirects=True
        )
    except httpx.HTTPError as exc:
...
def _fetch_openalex_extras(doi: str) -> dict:
    ...
    concepts = [
        c.get("display_name", "")
        for c in sorted(
            work.get("concepts") or [], key=lambda c: c.get("score") or 0, reverse=True
        )
...
def fetch_doi_metadata(doi: str) -> dict:
    ...
    if not re.match(r"^10\.\d{4,9}/\S+$", doi):
        raise SubmissionError(
            "That does not look like a valid DOI (expected e.g. 10.1234/abcd.5678)."
        )
...
def _name_parts(name: str) -> tuple[set[str], set[str], set[str]]:
    ...
    name = re.sub(
        r"^(dr|mr|ms|mrs|prof|professor|engr)\.?\s+", "", name.strip(), flags=re.I
    )
    raw = re.findall(r"[a-zA-Z]+", name.lower())
    full = {_VARIANTS.get(t, t) for t in raw if len(t) >= 2 and t not in _PARTICLES}
...
def verify_authorship(meta: dict, submitter: dict, submitter_orcid: str | None) -> str:
    ...
    """
    if submitter_orcid:
        paper_orcids = {_norm_orcid_id(o) for o in meta.get("author_orcids") or [] if o}
...
def _topics_for(
    title: str, abstract: str, concepts: list[str] | None = None
) -> list[dict]:
...
def _link_authors(author_names: list[str], submitter: dict) -> list[dict]:
    ...
    roster = {
        _name_key(r["full_name"]): r["researcher_id"]
    ...
    for order, name in enumerate(author_names[:12], start=1):
        ...
        if (
            rid is None
            and submitter["researcher_id"] not in linked_ids
...
def _persist(record: dict) -> dict:
    ...
    record["publication_id"] = max((p["publication_id"] for p in pubs), default=0) + 1
    pubs.append(record)
    pubs_path.write_text(json.dumps(pubs, indent=2, ensure_ascii=False), "utf-8")

    submitted_path = DATA_DIR / "submitted_publications.json"
    submitted = (
        json.loads(submitted_path.read_text("utf-8")) if submitted_path.exists() else []
    )
...
def _bump_researcher_counts(record: dict) -> None:
    ...
    for r in researchers:
        if r["researcher_id"] in linked:
            ...
            r["citation_count"] = r.get("citation_count", 0) + record.get(
                "citation_count", 0
            )
    path.write_text(json.dumps(researchers, indent=2, ensure_ascii=False), "utf-8")
    loader.clear_cache()
...
def preview_doi(
    doi: str, submitter: dict | None = None, submitter_orcid: str | None = None
) -> dict:
    ...
    topics = _topics_for(meta["title"], meta.get("abstract", ""), meta.get("concepts"))
...
def submit_doi(doi: str, submitter: dict, submitter_orcid: str | None = None) -> dict:
    ...
    if find_duplicate(meta["doi"], meta["title"]):
        raise SubmissionError("This publication is already in the database.")
    verify_authorship(meta, submitter, submitter_orcid)
...
def _create(meta: dict, submitter: dict, source: str) -> dict:
    ...
    if not any(a.get("researcher_id") == submitter["researcher_id"] for a in authors):
        authors.append(
            {
                "researcher_id": submitter["researcher_id"],
                "full_name": submitter["full_name"],
    ...
    record = {
        ...
        "authors": authors,
        "topics": _topics_for(
            meta["title"], meta.get("abstract", ""), meta.get("concepts")
        ),
        "source": source,
    }
